...
// "https://topdisc.ru/upload/...". Сам Bitrix ещё не знает, что его домен
// сменился на back.topdisc.ru (настройка "адрес сайта" в админке не
// обновлена), и topdisc.ru эти файлы больше не отдаёт вообще (проверено:
// соединение не устанавливается). Приводим такие ссылки к правильному
// абсолютному URL на back.topdisc.ru — картинки грузятся напрямую с
// бэкенда (не через nginx-прокси фронтенда: для <img src> CORS не нужен,
// а лишний прокси-хоп только замедляет — так и было раньше, до того как
// сертификат back.topdisc.ru не покрывал сам этот хост и пришлось на
