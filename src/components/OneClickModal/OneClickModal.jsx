...
function OneClickModal({ productName, productId, productPrice, onClose }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      fd.append('form_name', 'Заказ в 1 клик')
      fd.append('name', name.trim())
      fd.append('phone', '+' + phoneDigits)
      fd.append('product_id', productId)
      fd.append('product_name', productName)
      if (productPrice) fd.append('price', productPrice)
      await filterClient.post('/one-click', fd)
      setSent(true)
    } catch (err) {
      ...
      <div className="preorder-modal" onClick={(e) => e.stopPropagation()}>
        ...
        {sent ? (
          <div className="preorder-modal__success">
            ...
            <p>Мы перезвоним вам в ближайшее время, чтобы оформить заказ «{productName}».</p>
            <button className="preorder-modal__btn" onClick={onClose}>Закрыть</button>
          </div>
        ) : (
          <>
            <h3 className="preorder-modal__title">Купить в 1 клик</h3>
            <p className="preorder-modal__product">{productName}</p>
            <form className="preorder-modal__form" onSubmit={handleSubmit}>
              ...
              >
                {sending ? 'Отправка...' : 'Купить в 1 клик'}
              </button>
            </form>
          </>
        )}
