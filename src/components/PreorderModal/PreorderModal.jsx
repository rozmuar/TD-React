...
function PreorderModal({ productName, productId, onClose }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      fd.append('form_name', 'Хочу под заказ')
      fd.append('name', name.trim())
      fd.append('phone', '+' + phoneDigits)
      fd.append('product_id', productId)
      fd.append('product_name', productName)
      await filterClient.post('/preorder', fd)
      setSent(true)
    } catch (err) {
      ...
      <div className="preorder-modal" onClick={(e) => e.stopPropagation()}>
        ...
        {sent ? (
          <div className="preorder-modal__success">
            ...
            <p>Мы свяжемся с вами в ближайшее время по поводу товара «{productName}».</p>
            <button className="preorder-modal__btn" onClick={onClose}>Закрыть</button>
          </div>
        ) : (
          <>
            <h3 className="preorder-modal__title">Хочу под заказ</h3>
