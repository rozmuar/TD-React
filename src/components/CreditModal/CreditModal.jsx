...
function CreditModal({ productName, productId, productPrice, onClose }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      fd.append('form_name', 'Кредит / рассрочка')
      fd.append('name', name.trim())
      fd.append('phone', '+' + phoneDigits)
      fd.append('product_id', productId)
      fd.append('product_name', productName)
      fd.append('price', productPrice)
      await filterClient.post('/tinkoff-credit', fd)
      setSent(true)
    } catch (err) {
      ...
      <div className="preorder-modal" onClick={(e) => e.stopPropagation()}>
        ...
        {sent ? (
          <div className="preorder-modal__success">
            ...
            <p>Мы свяжемся с вами в ближайшее время для оформления кредита на товар «{productName}».</p>
            <button className="preorder-modal__btn" onClick={onClose}>Закрыть</button>
          </div>
        ) : (
          <>
            <h3 className="preorder-modal__title">Заполните контактные данные для оформления кредита</h3>
            <p className="preorder-modal__product">
              Вы оформляете в кредит: <strong>{productName}</strong>
            </p>
