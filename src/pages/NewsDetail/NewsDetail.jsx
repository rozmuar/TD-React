...
function NewsDetail() {
  ...
  const processDetailText = (html) => {
    ...
    return html
      // src у изображений уже относительные ("/upload/...") — оставляем как
      // есть, их резолвит nginx-прокси /upload/ на back.topdisc.ru; раньше
      // здесь стоял абсолютный префикс на topdisc.ru, но с переездом
      // Bitrix-бэкенда на back.topdisc.ru этот домен файлы больше не отдаёт
      // (см. память проекта, миграция API 2026-09-22), а back.topdisc.ru
      // напрямую из браузера не открыть из-за несовпадения SSL-сертификата
      //
      // href, наоборот, приводим к относительным — чтобы ссылки внутри
      // текста новости оставались в SPA-роутинге, а не улетали на бэкенд;
      // ловим оба варианта домена на случай, если в самом тексте уже
      // забит абсолютный адрес (старый topdisc.ru или новый back.topdisc.ru)
      .replace(/href="https?:\/\/(?:back\.)?topdisc\.ru\/([^"]+)"/g, 'href="/$1"')
      // Добавляем ID к h2
