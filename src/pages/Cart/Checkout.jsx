...
function Checkout() {
  ...
  const doCreateOrder = async () => {
    ...
    try {
      ...
      // !p.is_cash обязателен: у "Наличными при получении" платёж тоже
      // создаётся с paid=false (станет true только когда деньги примет
      // курьер/касса) — это не значит, что нужен переход на шлюз оплаты.
      const payment = data.payment?.find((p) => !p.paid && !p.is_cash)
      if (payment) {
